...
local function startBypassAimbot()
    ...
    bypassConn = RunService.Heartbeat:Connect(function()
        ...
        local char = LP.Character
        if not char then return end        local root = char:FindFirstChild("HumanoidRootPart")
        local hum = char:FindFirstChildOfClass("Humanoid")
        if not root or not hum then return end
...
-- 2. A função que o botão chama
local function executeTPDown()
    doTPDown()
end

-- 3. A função do Auto TP Down
...
local function buildGui()
    ...
    main.Position=UDim2.new(0,20,0,12)
    main.BackgroundColor3 = Color3.fromRGB(0,0,0)
    main.BackgroundTransparency = 0
    main.BorderSizePixel = 0
    main.ClipsDescendants = true
    
    -- CORNER PRINCIPAL COM CLIPSDESCENDANTS = TRUE (IMPORTANTE!)
    ...
    -- Canvas do sidebar (agora com ClipsDescendants = true e UICorner)
    ...
    -- Image com corner também (importante para cortar a imagem)
    ...
    sidebarImg.Image = "rbxassetid://133000221012609"
    sidebarImg.ScaleType = Enum.ScaleType.Crop
    sidebarImg.ClipsDescendants = true
    -- Aplicar o mesmo corner na imagem
    local imgCorner = Instance.new("UICorner", sidebarImg)
    imgCorner.CornerRadius = UDim.new(0, CORNER)
    ...
    brandFrame.Size = UDim2.new(1, -24, 0, 72)
    brandFrame.Position = UDim2.new(0, 12, 0, 6)
    brandFrame.BackgroundTransparency = 1
    brandFrame.ClipsDescendants = false
    ...
    brandLine.Size = UDim2.new(0, 100, 0, 2)
    brandLine.Position = UDim2.new(0.5, -50, 0, 24)
    brandLine.BackgroundColor3 = WHITE
    brandLine.BorderSizePixel = 0
    Instance.new("UICorner", brandLine).CornerRadius = UDim.new(1, 0)

    local linkLabel = Instance.new("TextLabel", brandFrame)
    linkLabel.Size = UDim2.new(1, 0, 0, 14)
    linkLabel.Position = UDim2.new(0, 0, 0, 32)
    linkLabel.BackgroundTransparency = 1
    linkLabel.Text = ".gg/UTM7FSRnjq"
    linkLabel.TextColor3 = Color3.fromRGB(200,200,200)
    linkLabel.Font = Enum.Font.Arcade
    linkLabel.TextSize = 9
    linkLabel.TextXAlignment = Enum.TextXAlignment.Center
    applyShimmerToText(linkLabel, 0.5)

    local brandSub = Instance.new("TextLabel", brandFrame)
    brandSub.Size = UDim2.new(1, 0, 0, 12)
    brandSub.Position = UDim2.new(0, 0, 0, 52)
    brandSub.BackgroundTransparency = 1
    brandSub.Text = "By Xluiz"
    brandSub.TextColor3 = Color3.fromRGB(180,180,180)
    ...
    tabContainer.Size = UDim2.new(0, 122, 0, 185)
    tabContainer.AnchorPoint = Vector2.new(0.5, 0)
    tabContainer.Position = UDim2.new(0.50, 0, 0, 82)
